...
import { sampleWithRequiredData } from '../timetable-version.test-samples';

import { TimetableVersion } from './timetable-version';

vitest.useFakeTimers();

describe('TimetableVersion Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: TimetableVersion;
  let fixture: ComponentFixture<TimetableVersion>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(TimetableVersion);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 27241 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.timetableVersions()[0]).toEqual(expect.objectContaining({ id: 27241 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 27241 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.timetableVersions()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 27241 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to timetableVersionService', () => {
      ...
      vitest.spyOn(service, 'getTimetableVersionIdentifier');
      const id = comp.trackId(entity);
      expect(service.getTimetableVersionIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.timetableVersionsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
