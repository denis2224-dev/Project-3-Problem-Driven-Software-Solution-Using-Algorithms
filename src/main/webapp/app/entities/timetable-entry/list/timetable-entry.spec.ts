...
import { sampleWithRequiredData } from '../timetable-entry.test-samples';

import { TimetableEntry } from './timetable-entry';

vitest.useFakeTimers();

describe('TimetableEntry Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: TimetableEntry;
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 15795 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.timetableEntries()[0]).toEqual(expect.objectContaining({ id: 15795 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 15795 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.timetableEntries()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 15795 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to timetableEntryService', () => {
      const entity = { id: 15795 };
      vitest.spyOn(service, 'getTimetableEntryIdentifier');
      const id = comp.trackId(entity);
      expect(service.getTimetableEntryIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.timetableEntriesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
