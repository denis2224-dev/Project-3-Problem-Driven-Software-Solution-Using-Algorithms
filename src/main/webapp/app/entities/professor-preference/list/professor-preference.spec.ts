...
import { sampleWithRequiredData } from '../professor-preference.test-samples';
import { ProfessorPreferenceService } from '../service/professor-preference.service';

import { ProfessorPreference } from './professor-preference';

vitest.useFakeTimers();

describe('ProfessorPreference Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: ProfessorPreference;
  let fixture: ComponentFixture<ProfessorPreference>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(ProfessorPreference);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 6094 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.professorPreferences()[0]).toEqual(expect.objectContaining({ id: 6094 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 6094 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.professorPreferences()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 6094 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to professorPreferenceService', () => {
      const entity = { id: 6094 };
      vitest.spyOn(service, 'getProfessorPreferenceIdentifier');
      const id = comp.trackId(entity);
      expect(service.getProfessorPreferenceIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.professorPreferencesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
