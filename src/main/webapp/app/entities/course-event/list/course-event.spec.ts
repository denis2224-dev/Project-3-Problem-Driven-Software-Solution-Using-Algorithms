...
import { sampleWithRequiredData } from '../course-event.test-samples';
...
describe('CourseEvent Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: CourseEvent;
  let fixture: ComponentFixture<CourseEvent>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(CourseEvent);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 9540 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.courseEvents()[0]).toEqual(expect.objectContaining({ id: 9540 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 9540 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.courseEvents()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 9540 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to courseEventService', () => {
      ...
      vitest.spyOn(service, 'getCourseEventIdentifier');
      const id = comp.trackId(entity);
      expect(service.getCourseEventIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.courseEventsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
