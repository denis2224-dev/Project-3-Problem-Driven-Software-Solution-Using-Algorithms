...
import { sampleWithRequiredData } from '../schedule-conflict.test-samples';
...
describe('ScheduleConflict Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: ScheduleConflict;
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 30168 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.scheduleConflicts()[0]).toEqual(expect.objectContaining({ id: 30168 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 30168 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.scheduleConflicts()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 30168 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to scheduleConflictService', () => {
      const entity = { id: 30168 };
      vitest.spyOn(service, 'getScheduleConflictIdentifier');
      const id = comp.trackId(entity);
      expect(service.getScheduleConflictIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.scheduleConflictsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
