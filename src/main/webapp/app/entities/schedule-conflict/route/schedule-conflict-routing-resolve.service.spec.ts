...
import scheduleConflictResolve from './schedule-conflict-routing-resolve.service';

describe('ScheduleConflict routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IScheduleConflict returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          scheduleConflictResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(scheduleConflictResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
