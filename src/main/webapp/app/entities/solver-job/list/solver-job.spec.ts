...
import { sampleWithRequiredData } from '../solver-job.test-samples';

import { SolverJob } from './solver-job';

vitest.useFakeTimers();

describe('SolverJob Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: SolverJob;
  let fixture: ComponentFixture<SolverJob>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(SolverJob);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 6466 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.solverJobs()[0]).toEqual(expect.objectContaining({ id: 6466 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 6466 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.solverJobs()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 6466 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to solverJobService', () => {
      ...
      vitest.spyOn(service, 'getSolverJobIdentifier');
      const id = comp.trackId(entity);
      expect(service.getSolverJobIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.solverJobsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
