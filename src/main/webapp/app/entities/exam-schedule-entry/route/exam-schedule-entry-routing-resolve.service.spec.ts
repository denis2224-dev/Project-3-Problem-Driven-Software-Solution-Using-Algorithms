...
import examScheduleEntryResolve from './exam-schedule-entry-routing-resolve.service';

describe('ExamScheduleEntry routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IExamScheduleEntry returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          examScheduleEntryResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(examScheduleEntryResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
