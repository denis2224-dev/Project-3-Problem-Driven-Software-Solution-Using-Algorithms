...
import { sampleWithRequiredData } from '../exam-schedule-entry.test-samples';
import { ExamScheduleEntryService } from '../service/exam-schedule-entry.service';

import { ExamScheduleEntry } from './exam-schedule-entry';

vitest.useFakeTimers();

describe('ExamScheduleEntry Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: ExamScheduleEntry;
  let fixture: ComponentFixture<ExamScheduleEntry>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(ExamScheduleEntry);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 619 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.examScheduleEntries()[0]).toEqual(expect.objectContaining({ id: 619 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 619 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.examScheduleEntries()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 619 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to examScheduleEntryService', () => {
      const entity = { id: 619 };
      vitest.spyOn(service, 'getExamScheduleEntryIdentifier');
      const id = comp.trackId(entity);
      expect(service.getExamScheduleEntryIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.examScheduleEntriesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
