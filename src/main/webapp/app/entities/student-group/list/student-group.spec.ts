...
import { sampleWithRequiredData } from '../student-group.test-samples';
...
describe('StudentGroup Management Component', () => {
  ...
  let comp: StudentGroup;
  let fixture: ComponentFixture<StudentGroup>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(StudentGroup);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 20706 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.studentGroups()[0]).toEqual(expect.objectContaining({ id: 20706 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 20706 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.studentGroups()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 20706 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to studentGroupService', () => {
      ...
      vitest.spyOn(service, 'getStudentGroupIdentifier');
      const id = comp.trackId(entity);
      expect(service.getStudentGroupIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.studentGroupsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
