...
public interface ProfessorRepository extends JpaRepository<Professor, Long> {
    ...
    @Query(
        value = "select professor from Professor professor left join fetch professor.department",
        countQuery = "select count(professor) from Professor professor"
    )
    Page<Professor> findAllWithToOneRelationships(Pageable pageable);

    @Query("select professor from Professor professor left join fetch professor.department")
    List<Professor> findAllWithToOneRelationships();

    @Query("select professor from Professor professor left join fetch professor.department where professor.id =:id")
