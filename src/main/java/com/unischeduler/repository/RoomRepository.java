...
public interface RoomRepository extends JpaRepository<Room, Long> {
    ...
    @Query(value = "select room from Room room left join fetch room.building", countQuery = "select count(room) from Room room")
    Page<Room> findAllWithToOneRelationships(Pageable pageable);

    @Query("select room from Room room left join fetch room.building")
    List<Room> findAllWithToOneRelationships();

    @Query("select room from Room room left join fetch room.building where room.id =:id")
