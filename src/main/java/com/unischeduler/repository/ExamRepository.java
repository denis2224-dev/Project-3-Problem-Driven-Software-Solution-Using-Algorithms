...
public interface ExamRepository extends JpaRepository<Exam, Long> {
    ...
    @Query(
        value = "select exam from Exam exam left join fetch exam.course left join fetch exam.studentGroup",
        countQuery = "select count(exam) from Exam exam"
    )
    Page<Exam> findAllWithToOneRelationships(Pageable pageable);

    @Query("select exam from Exam exam left join fetch exam.course left join fetch exam.studentGroup")
    List<Exam> findAllWithToOneRelationships();

    @Query("select exam from Exam exam left join fetch exam.course left join fetch exam.studentGroup where exam.id =:id")
