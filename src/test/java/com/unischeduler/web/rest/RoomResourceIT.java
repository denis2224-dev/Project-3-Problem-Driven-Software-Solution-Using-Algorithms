...
import com.unischeduler.service.mapper.RoomMapper;
...
class RoomResourceIT {
    ...
    private static final RoomType DEFAULT_ROOM_TYPE = RoomType.LECTURE;
    private static final RoomType UPDATED_ROOM_TYPE = RoomType.LABORATORY;
    ...
    public static Room createEntity(EntityManager em) {
        ...
        if (TestUtil.findAll(em, Building.class).isEmpty()) {
            building = BuildingResourceIT.createEntity();
            em.persist(building);
            em.flush();
        } else {
    ...
    public static Room createUpdatedEntity(EntityManager em) {
        ...
        Building building;
        if (TestUtil.findAll(em, Building.class).isEmpty()) {
            building = BuildingResourceIT.createUpdatedEntity();
            em.persist(building);
            em.flush();
        } else {
            building = TestUtil.findAll(em, Building.class).get(0);
    ...
    void partialUpdateRoomWithPatch() throws Exception {
        ...
        partialUpdatedRoom.roomType(UPDATED_ROOM_TYPE).equipment(UPDATED_EQUIPMENT);
